...
with open("typechart.json") as f:
    TYPECHART = json.load(f)

TYPE_MULT = {0: 1, 1: 2, 2: 0.5, 3: 0}  # Showdown's damageTaken codes
...
def calc_stat(base: int, is_hp: bool = False, level: int = 100, iv: int = 31, ev: int = 0, nature: float = 1.0) -> int:
    """Approximates a real stat from base stat — assumes neutral nature/EVs unless told otherwise."""
...
def estimate_damage_pct(attacker_species, attacker_stats, move_name, defender_species, level=100, atk_boost=0, def_boost=0):
    """Returns (low%, high%) of defender's max HP, or None if not a damaging move."""
    ...
    max_hp_est = int((2 * def_hp + 31) * level / 100) + level + 10  # assumes neutral IVs, 0 EVs
...
def analyze_matchup(state: dict) -> dict:
    ...
    opp_hp_pct = (opp["hp"] / opp["maxhp"] * 100) if opp.get("maxhp") else 100
    ...
    status_moves = [
        (m["move"] if isinstance(m, dict) else m)
        for m in my_moves
        if MOVES.get(_id(m["move"] if isinstance(m, dict) else m), {}).get("category") == "Status"
    ]

    best_my_move = max(my_options, key=lambda x: x[1][1], default=None)
    can_i_ko = bool(best_my_move and best_my_move[1][0] >= opp_hp_pct)
    ...
    can_opp_ko = bool(best_opp_move and my_hp_pct is not None and best_opp_move[1][0] >= my_hp_pct)
