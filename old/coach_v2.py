...
If a fact marked CRITICAL is present, it overrides all other considerations — do not recommend attacking if a CRITICAL warning says you won't survive to take your turn. When switching is advised, name specific healthy bench Pokemon from the bench team list above.
...
Don't default to caution — if you are not at risk of fainting this turn (no CRITICAL warning present), actively look for a good setup or status-move opportunity (such as using setup moves listed above) rather than only considering switch/attack. A safe turn is a good turn to invest in boosts.
